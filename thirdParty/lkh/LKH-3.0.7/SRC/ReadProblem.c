...
void ReadProblem() {
  ...
  if (CostMatrix == 0 && Dimension <= MaxMatrixDimension && Distance != 0 &&
      Distance != Distance_1 && Distance != Distance_EXPLICIT &&
  ...
  if (ProblemType == TSPTW || ProblemType == CVRPTW || ProblemType == VRPBTW ||
  ...
  if (ProblemType != CVRP && ProblemType != CVRPTW && ProblemType != CTSP &&
      ProblemType != STTSP && ProblemType != TSP && ProblemType != ATSP) {
    M = INT_MAX / 2 / Precision;
    for (i = Dim + 1; i <= DimensionSaved; i++) {
...
static int TwoDWeightType() {
  if (Asymmetric)
    return 0;
  return WeightType == EUC_2D || WeightType == MAX_2D || WeightType == MAN_2D ||
         WeightType == CEIL_2D || WeightType == FLOOR_2D || WeightType == GEO ||
         WeightType == GEOM || WeightType == GEO_MEEUS ||
         WeightType == GEOM_MEEUS || WeightType == ATT ||
         WeightType == TOR_2D ||
...
static int ThreeDWeightType() {
  if (Asymmetric)
    return 0;
  return WeightType == EUC_3D || WeightType == MAX_3D || WeightType == MAN_3D ||
         WeightType == CEIL_3D || WeightType == FLOOR_3D ||
         WeightType == TOR_3D || WeightType == XRAY1 || WeightType == XRAY2 ||
...
static void CheckSpecificationPart() {
  ...
  if (WeightType != EXPLICIT &&
    ...
    eprintf("Conflicting EDGE_WEIGHT_TYPE and EDGE_WEIGHT_FORMAT");
  if ((ProblemType == ATSP || ProblemType == SOP) && WeightType != EXPLICIT &&
      WeightType != -1)
  ...
  if (ExtraCandidateSetType == QUADRANT && !TwoDWeightType() &&
    ...
    eprintf("Illegal EDGE_WEIGHT_TYPE for EXTRA_CANDIDATE_SET_TYPE = "
            "QUADRANT");
  if (InitialTourAlgorithm == QUICK_BORUVKA && !TwoDWeightType() &&
      !ThreeDWeightType())
    eprintf("Illegal EDGE_WEIGHT_TYPE for INITIAL_TOUR_ALGORITHM = "
            "QUICK-BORUVKA");
  ...
  for (i = 1; i <= Dim; i++) {
    ...
    if (N->V == 1)
      eprintf("PICKUP_AND_DELIVERY_SECTION: "
              ...
              N->Id);
    N->V = 1;
    if (!fscanf(ProblemFile, "%d %lf %lf %lf %d %d", &N->Demand, &N->Earliest,
                &N->Latest, &N->ServiceTime, &N->Pickup, &N->Delivery))
      eprintf("PICKUP_AND_DELIVERY_SECTION: "
...
static void Read_TOUR_SECTION(FILE **File) {
  ...
  if (File == &SubproblemTourFile) {
    do {
      if (N->FixedTo1 && N->SubproblemPred != N->FixedTo1 &&
...
static void Read_TYPE() {
  ...
  if (!strcmp(Type, "TSP"))
    ...
    ProblemType = VRPBTW;
  else if (!strcmp(Type, "VRPSPD") || !strcmp(Type, "VRPSPDTW") ||
           !strcmp(Type, "VRPMPD") || !strcmp(Type, "VRPMPDTW") ||
           !strcmp(Type, "MVRPB"))
    ProblemType = VRPPD;
  ...
  Asymmetric =
      ProblemType == ATSP || ProblemType == CCVRP || ProblemType == ACVRP ||
      ProblemType == ADCVRP || ProblemType == CVRPTW || ProblemType == MLP ||
      ProblemType == M_PDTSP || ProblemType == M1_PDTSP ||
      ProblemType == ONE_PDTSP || ProblemType == OVRP || ProblemType == PDTSP ||
      ProblemType == PDTSPF || ProblemType == PDTSPL || ProblemType == PDPTW ||
      ProblemType == RCTVRP || ProblemType == RCTVRPTW || ProblemType == SOP ||
      ProblemType == TRP || ProblemType == TSPDL || ProblemType == TSPTW ||
...
void ReadTour(char *FileName, FILE **File) {
  ...
  while ((Line = ReadLine(*File))) {
    ...
    for (i = 0; i < strlen(Keyword); i++)
      Keyword[i] = (char)toupper(Keyword[i]);
    if (!strcmp(Keyword, "COMMENT") || !strcmp(Keyword, "DEMAND_SECTION") ||
        ...
        !strcmp(Keyword, "FIXED_EDGES_SECTION") || !strcmp(Keyword, "NAME") ||
